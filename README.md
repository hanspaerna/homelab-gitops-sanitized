...
In Netbird dashboard (netbird.lab.com), create an access token, put it into Ansible's "inventory.tf.yml".

Back to playbooks:

5. netbird-install.yml

In the end, you will get a temporary file called "netbird-setup-keys.txt" with short-lived setup keys for everything. 

The clusters will connect to NetBird automatically afterwards (you don't need to do anything with k3s-int and k3s-ext keys at all), but it can take a few minutes. 
Kill netbird-router pods manually in both clusters if you don't want to wait.

When two clusters are connected, change the internal cluster's (k3s-int) IP to 100.116.56.43 in the dashboard. 
If there's no connection after that, delete netbird-router pod on int; it will be created again.
...
Just as with all other things hosted on k3s-ext, please don't try to make any permanent changes via NetBird's management UI. 
Always change the declarative state in "./ansible/netbird-configuration", then run "netbird-apply.yml" playbook.
Every resource that is not a part of the repo will be removed from NetBird (strict=true).
