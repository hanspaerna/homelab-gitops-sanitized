...
<img src="docs/infrastructure.drawio.png">

## About

**Hypervisor**: Proxmox VE <br />
**NAS**: ZFS on Proxmox over NFSv4 <br />
...
**Volume backups**: k8up (sends volume backups to local Restic REST Server) <br />
...
```
You should have two A records already in Blocky DNS configuration:
...
All MagicDNS hostnames are resolvable from wg-easy clients, as MagicDNS "server" (100.100.100.100) is set as as the only one for any new plain-wg connection.
So the only functional difference is in the relayed connection being guaranteed, with no attempts of connecting directly to peer from a phone.
...
Also run this to avoid RFC1918 error when accessing LuCI via tailnet: `uci set uhttpd.main.rfc1918_filter=0 && uci commit uhttpd && /etc/init.d/uhttpd restart`

Don't forget to run all clients with --login-server (and --accept-routes for Linux) parameters. Add --accept-dns=true, if access to other nodes via MagicDNS is needed.
Linux clients will not accept routes from other subnet routers by default, so wg-easy clients will not be able to interact with this new tailnet node without the parameter.
...
- WG_POST_UP and WG_POST_DOWN in wg-easy cannot be automatically set via env vars since v15, has to be manually changed via the admin UI.